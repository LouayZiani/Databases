...
VALUES (1, 1, 101, CURRENT_DATE, 2); --current date since the order is placed now
...
--2: Transaction w/ Rollback
BEGIN;
DO $$
BEGIN
	-- Checking if quantity in books in sufficient
...
SELECT price FROM Books WHERE book_id = 1; -- re-reading the price after commiting
...
SELECT email FROM Customers WHERE customer_id = 101; -- even after i restarted the db server, the new 
													-- email persists